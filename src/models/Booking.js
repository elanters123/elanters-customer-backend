...
const bookingSchema = new Schema({
  ...
  eOrderId: { type: String, default: "Elanters", trim: true },
  ...
  customer: {
    // id: { type: Types.ObjectId, ref: 'customer', required: true }, // Temporarily removed required
    id: { type: Types.ObjectId, ref: 'customer' }, // Make optional for testing - ref must match model name (lowercase)
  ...
  location: {
    ...
    // city: { type: String, required: true }, // Temporarily removed required
    city: { type: String }, // Make optional for testing
    // state: { type: String, required: true }, // Temporarily removed required
    state: { type: String }, // Make optional for testing
    // postalCode: { type: String, required: true }, // Temporarily removed required
    postalCode: { type: String }, // Make optional for testing
    coordinates: {
  ...
  payment: {
    ...
    companyShare: { type: Number, required: false, default: 0 },
    gardenerShare: { type: Number, required: false, default: 0 },
    status: {
      type: String,
...
// Ensure the model name matches what you use elsewhere (e.g., 'Booking')
